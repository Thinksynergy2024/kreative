...
const ActiveTestGraph = () => {
  const { tests } = useSelector((store) => store.test);

  // Calculate counts for each phase
  const phaseOneCount = tests.filter((test) => test.phase === "Phase 1").length;
  const phaseTwoCount = tests.filter((test) => test.phase === "Phase 2").length;
  const phaseThreeCount = tests.filter((test) => test.phase === "Phase 3").length;
  const phaseFourCount = tests.filter((test) => test.phase === "Phase 4").length;
  const phaseFiveCount = tests.filter((test) => test.phase === "Phase 5").length;
...
export default ActiveTestGraph;
