...
import { createTest } from "@/redux/service";
...
const CreateTest = () => {
  ...
  const handleCreateTest = async (formValue, helpers) => {
    const option = "varietycode";
    try {
      setLoading(true);
      const res = await createTest(formValue.bar_code, option);
      dispatch(setTests(res));
      helpers.resetForm();
      setLoading(false);
      toast.success("Bar code sent successfully");
      // router.push("/dashboard/create-test/varities");
    } catch (err) {
      toast.error(err);
    }
  };
